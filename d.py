...
class Border:
    ...
    def draw_on(self, draw, pos=(0, 0)):
        if self.border_type != 'none':
            draw.rectangle(self.border_rect(draw), outline=self.border_type)

    def border_rect(self, draw):
...
class Align:
    ...
    def Centre(display, message_size):
        message_y = (display.size[0] - message_size[0]) / 2
        message_x = (display.size[1] - message_size[1]) / 2
